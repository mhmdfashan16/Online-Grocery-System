...
const Cart = () => {
    ...
    const {products,axios, currency, cartItems, removeFromCart, getCartCount, updateCartItems, navigate, getCartAmount, deliveryFee, user, setCartItems} = useAppContext();
    ...
    const placeOrder = async()=>{
        try{
            ...
            if(paymentOption === "COD"){
                ...
                //place orcder with stripe : /api/order/stripe
                        ...
                        <div className="flex items-center md:gap-6 gap-3">
                            ...
                            <div>
                                ...
                                <div className="font-normal text-gray-500/70">
                                    <p>Weigth: <span>{product.weight || "N/A"}</span></p>
                                    <div className='flex items-center'>
                                        ...
                                        <select onChange={e => updateCartItems(product._id, Number(e.target.value))} value={cartItems[product._id]} className='outline-none'>
                                            {Array(cartItems[product._id] > 9 ? cartItems[product._id] : 9).fill('').map((_, index) => (
                                                <option key={index} value={index + 1} className="cursor-pointer">{index + 1}</option>
                                            ))}                                          
                                        </select>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {addresses.map((adress, index)=>(<p onClick={() =>{setSelectedAddress(adress); setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
                ...
                <div className="text-gray-500 mt-4 space-y-2">
                    ...
                    <p className="flex justify-between text-lg font-medium mt-3">
                        <span>
                            Total Amount:</span><span>{currency}{getCartAmount() + getCartAmount()* 2/100 +deliveryFee}    
                        </span>
                    </p>
                </div>
